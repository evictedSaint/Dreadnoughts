...
public class GuiCompDefenseStatic {

	private JFrame frmStaticDefense;
	...
	public static DefenseStatic ds = new DefenseStatic(null);
	 ...
	 */
	public GuiCompDefenseStatic() {
		initialize();
		frmStaticDefense.setVisible(true);
	}
	...
	private void initialize() { // TODO: all of these need a price at some point
		frmStaticDefense = new JFrame();
		frmStaticDefense.setTitle("Static Defense");
		//frmStaticDefense.setAlwaysOnTop(true);
		frmStaticDefense.setBounds(100, 100, 505, 679);
		frmStaticDefense.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frmStaticDefense.getContentPane().setLayout(new MigLayout("", "[grow]", "[][][grow][bottom]"));

		JPanel panelCompValues = new JPanel();
		frmStaticDefense.getContentPane().add(panelCompValues, "cell 0 0,alignx left,aligny top");
		...
		frmStaticDefense.getContentPane().add(panel_Cost_Factions, "cell 0 1,grow");
		...
		frmStaticDefense.getContentPane().add(panel_3, "cell 0 2,grow");
		...
		JLabel lblNewLabel_5 = new JLabel("Uses Per Turn:");
		panel_5.add(lblNewLabel_5, "cell 0 0");

		JSpinner spinner_uses = new JSpinner();
		spinner_uses.setModel(new SpinnerNumberModel(new Integer(1), new Integer(0), null, new Integer(1)));
		panel_5.add(spinner_uses, "cell 1 0,growx");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if (performanceModel.size() < list_Status.getModel().getSize()) {
					new GuiStaticDefenseWindow(ds,(Status) list_Status.getModel().getElementAt(performanceModel.size()));
				}
			}
		});
		...
		frmStaticDefense.getContentPane().add(panel_6, "cell 0 3,growx");
		panel_6.setLayout(new BorderLayout(0, 0));

		JButton btnNewButton_1 = new JButton("Cancel");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				frmStaticDefense.dispatchEvent(new WindowEvent(frmStaticDefense, WindowEvent.WINDOW_CLOSING));
			}
		});
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (performanceModel.size() < list_Status.getModel().getSize()) {
					JOptionPane.showMessageDialog(new JFrame(), "Not all values entered");
					return;
				}
				...
				ds.componentName = textFieldCompName.getText().trim();
				try {
				...
				ds.powerRequirement = (int) spinnerPowerReq.getValue(); // wrapped in an Object type, so we need to cast
																		// it
				ds.canBeDamaged = rdoDamageableTrue.isSelected();
				ds.repairable = rdoRepairableTrue.isSelected();
				ds.augmentSlot = rdoAugmentTrue.isSelected();
				ds.commons = (int) spinner_C.getValue();
				ds.energetics = (int) spinner_E.getValue();
				ds.xotics = (int) spinner_X.getValue();
				ds.owningFactions = (List<String>) list_factions.getSelectedValuesList();

				// defenserolling specific values next...
				ds.capacity = (int) spinner_uses.getValue();
				
				GuiNewGameMode.addComponent(ds);
	...
	public static void updatePerformanceList() {
		System.out.println(performanceModel.size());
