...
public class GuiCompDefenseBank {

	private JFrame frmBankDefense;
	...
	private JTextField textField_MaxCharge;
	private JTextField textField_Recharge;
	 ...
	 */
	public GuiCompDefenseBank() {
		initialize();
		frmBankDefense.setVisible(true);
	}
	...
	private void initialize() { //TODO: all of these need a price at some point
		frmBankDefense = new JFrame();
		frmBankDefense.setTitle("Bank Defense");
		//frmBankDefense.setAlwaysOnTop(true);
		frmBankDefense.setBounds(100, 100, 505, 782);
		frmBankDefense.setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		frmBankDefense.getContentPane().setLayout(new MigLayout("", "[grow]", "[][][grow][bottom]"));
		
		JPanel panelCompValues = new JPanel();
		frmBankDefense.getContentPane().add(panelCompValues, "cell 0 0,alignx left,aligny top");
		...
		frmBankDefense.getContentPane().add(panel_Cost_Factions, "cell 0 1,grow");
		...
		frmBankDefense.getContentPane().add(panel_3, "cell 0 2,grow");
		...
		JLabel lblNewLabel_5 = new JLabel("Defense Resource");
		panel_5.add(lblNewLabel_5, "cell 0 0,alignx trailing");
		
		JComboBox<String> comboBox = new JComboBox<String>();
		for (String defRes : GameMode.getInstance().bankDefenseProjectileTypeDamage.keySet()) {
			comboBox.addItem(defRes);
		}
		...
		JLabel lblNewLabel_8 = new JLabel("Recharge/Turn");
		...
		DefaultListModel<Integer> chargeModel = new DefaultListModel<>();
		JList<Integer> list_Charge = new JList<Integer>(chargeModel);
		list_Charge.setEnabled(false);
		panel_4.add(list_Charge, "cell 1 1,grow");

		DefaultListModel<Integer> rechargeModel = new DefaultListModel<>();
		JList<Integer> list_Recharge = new JList<Integer>(rechargeModel);
		list_Recharge.setEnabled(false);
		panel_4.add(list_Recharge, "cell 2 1,grow");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				try {
					int maxCharge = Integer.parseInt(textField_MaxCharge.getText());
					int recharge = Integer.parseInt(textField_Recharge.getText());
					
					if (rechargeModel.size() < list_Status.getModel().getSize() && chargeModel.size() < list_Status.getModel().getSize()) {
						rechargeModel.addElement(recharge);
						chargeModel.addElement(maxCharge);
					}
					textField_MaxCharge.setText("");
					textField_Recharge.setText("");
				}
				catch (Exception e) {
		...
		btnNewButton_3.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (chargeModel.size() > 1 && rechargeModel.size() > 1) {
					chargeModel.remove(chargeModel.size()-1);
					rechargeModel.remove(rechargeModel.size()-1);
				}
			}
		});
		...
		textField_MaxCharge = new JTextField();
		panel_4.add(textField_MaxCharge, "cell 1 2,growx");
		textField_MaxCharge.setColumns(10);
		
		textField_Recharge = new JTextField();
		panel_4.add(textField_Recharge, "cell 2 2,growx");
		textField_Recharge.setColumns(10);
		
		JPanel panel_6 = new JPanel();
		frmBankDefense.getContentPane().add(panel_6, "cell 0 3,growx");
		panel_6.setLayout(new BorderLayout(0, 0));
		
		JButton btnNewButton_1 = new JButton("Cancel");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				frmBankDefense.dispatchEvent(new WindowEvent(frmBankDefense, WindowEvent.WINDOW_CLOSING));
			}
		});
		...
		btnNewButton_2.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (comboBox.getSelectedItem() == null) {
					JOptionPane.showMessageDialog(new JFrame(), "No Defense Resource selected");
					return;
				}

				if (list_Status.getModel().getSize() != chargeModel.size() || list_Status.getModel().getSize() != rechargeModel.size()) {
					JOptionPane.showMessageDialog(new JFrame(), "Invalid Recharge and Charge values");
					return;
				}
				...
				DefenseBank db = new DefenseBank(textFieldCompName.getText().trim());
				...
				db.powerRequirement = (int) spinnerPowerReq.getValue(); //wrapped in an Object type, so we need to cast it
				db.canBeDamaged = rdoDamageableTrue.isSelected();
				db.repairable = rdoRepairableTrue.isSelected();
				db.augmentSlot = rdoAugmentTrue.isSelected();
				db.commons = (int) spinner_C.getValue();
				db.energetics = (int) spinner_E.getValue();
				db.xotics = (int) spinner_X.getValue();
				db.owningFactions = (List<String>) list_factions.getSelectedValuesList();
				
				
				
				//defensebank specific values next...
				db.defenseResource = (String) comboBox.getSelectedItem(); 
				for (int i = 0; i < list_Status.getModel().getSize(); i++) {
					db.maxValue.put((Status) list_Status.getModel().getElementAt(i), chargeModel.getElementAt(i));
					db.perTurnRecharge.put((Status) list_Status.getModel().getElementAt(i), rechargeModel.getElementAt(i));
				}
				
				GuiNewGameMode.addComponent(db);
