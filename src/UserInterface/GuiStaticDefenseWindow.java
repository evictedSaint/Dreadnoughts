...
public class GuiStaticDefenseWindow {

	private JFrame frame;
	private Status statusName;
	private DefenseStatic ds;
	private JTextField textProjectileDamage;

	/**
	 * Create the application.
	 */
	public GuiStaticDefenseWindow(DefenseStatic ds, Status status) {
	...
	private void initialize() {
		...
		panel_3_.setLayout(new MigLayout("", "[0,grow][grow]", "[20px][217px,grow][26px]"));
		...
		JLabel lblNewLabel_4 = new JLabel("(Note: High Damage=Better Defense)");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (projDamModel.size() > 0) {
					ds.projectileTypeDamage.get(statusName).put(weaponTypesModel.elementAt(projDamModel.getSize()-1), 0);
					projDamModel.remove(projDamModel.size()-1);
				}
			}
		});
		...
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					if (projDamModel.size() < weaponTypesModel.size()) {
						projDamModel.addElement(textProjectileDamage.getText());
						textProjectileDamage.setText("");
						ds.projectileTypeDamage.get(statusName).put(weaponTypesModel.elementAt(weaponTypesModel.getSize()-1), damageToProj);
					}
				}
		...
		btnFinish.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (weaponTypesModel.size() != projDamModel.size()) {
					JOptionPane.showMessageDialog(new JFrame(), "Invalid damage values");
					return;
				}
				GuiCompDefenseStatic.updatePerformanceList();
